...
import { FilesystemServer } from "./filesystem_server";
import {
  MCPRequest,
  MCPResponse,
  MCP_ERROR_CODES,
...
const PORT = parseInt(process.env.MCP_FILESYSTEM_PORT || "9322", 10);
const HOST = process.env.MCP_FILESYSTEM_HOST || "localhost";
...
const httpServer = http.createServer((req, res) => {
  ...
  if (req.method === "GET" && parsedUrl.pathname === "/health") {
    ...
    res.end(JSON.stringify({ status: "ok", server: "mcp-filesystem" }));
    return;
  }
...
process.on("SIGINT", () => {
  console.log("\nShutting down MCP Filesystem Server...");
  httpServer.close();
  process.exit(0);
});
